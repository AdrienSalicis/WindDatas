...
def get_meteostat_data(site_name, lat, lon, start, end):
    try:
        ...
        # Sélectionner uniquement les variables de vent
...
# ------------------------------
# Récupère les données Openmeteo
# ------------------------------
...
def get_openmeteo_data(site_name, lat, lon, start, end):
    try:
        ...
        # Requête pour récupérer plus de données de vent
        url = (
            f"https://archive-api.open-meteo.com/v1/archive"
            f"?latitude={lat}&longitude={lon}"
            f"&start_date={start_str}&end_date={end_str}"
            f"&daily=windspeed_10m_max,windspeed_10m_mean,windgusts_10m_max,winddirection_10m_max"
            f"&timezone=auto"
        )
        ...
        if "daily" not in json_data:
            print(f"[ERROR] Aucune donnée pour {site_name} entre {start_str} et {end_str}")
            return None
        
        # Transformation des données JSON en DataFrame
        df = pd.DataFrame({
            ...
            "wind_gust_max": json_data["daily"]["windgusts_10m_max"],
            "wind_direction_max": json_data["daily"]["winddirection_10m_max"]
        })
